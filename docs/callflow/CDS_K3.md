...
> K4 와 같은 구조. 다만 준비용 K1 의 `START_TIME` 이 **현재 시각**이다 — 유효기간이 찬 쿠폰이 필요하기 때문이다.
...
- **SBI Noti 도착** — `Verify SBI Noti Sent K3` (위 PDB 판정과 함께 본다)
...
- `tests/cds/cds_tests.robot` — `TC-CDS-014`
