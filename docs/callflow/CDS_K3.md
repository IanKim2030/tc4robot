# CDS `K3` — Data(Time) 쿠폰 만료

| 항목 | 값 |
|---|---|
| 업무 코드 | `K3` |
| 테스트 케이스 | **없음** — 이 코드를 보내는 TC 가 아직 없다 |
| 알림 경로 | SDM 경유 — 규칙 3 |
...
> 준비용 K1 의 `START_TIME` 이 **현재 시각**이어야 한다 — 유효기간이 찬 쿠폰이 필요하기 때문이다. "0건" 이 "만료됐다" 인지 "원래 없었다" 인지 구분되지 않으므로, TC 를 만들 때는 자기 핀으로 가입을 먼저 만들어야 한다.
...
sequenceDiagram
    ...
    TOOL->>PCDS: 0015 CommandRequest (K3) — Body 327B
...
알림은 `Verify SBI Noti Sent K3` 가 본다(도착 여부). 경로는 수신만으로 구분되지 않아 규칙으로 계산해 실패 메시지에 싣는다.

## 관련 문서

- [CDS 노드 스펙](../nodes/CDS.md) — 인코딩 표, 함정, LTE/SA 차이
- `tests/cds/cds_tests.robot` — **해당 TC 없음** (전문·판정 기준만 정리해 둔 시트다)
