...
> TC 안에서 K1 으로 먼저 가입시킨 뒤 취소한다 — "0건" 이 "지워졌다" 인지 "원래 없었다" 인지 구분되지 않기 때문이다.
...
sequenceDiagram
    ...
    TOOL->>PCDS: 0015 CommandRequest (K4) — Body 327B
...
- **SBI Noti 도착** — `Verify SBI Noti Sent K4` (위 PDB 판정과 함께 본다)

알림은 `Verify SBI Noti Sent K4` 가 본다(도착 여부). 경로는 수신만으로 구분되지 않아 규칙으로 계산해 실패 메시지에 싣는다.

## 관련 문서

- [CDS 노드 스펙](../nodes/CDS.md) — 인코딩 표, 함정, LTE/SA 차이
- [1X 전문 End-to-End](CDS_X1.md) — PG 내부 프로세스 상세
- `tests/cds/cds_tests.robot` — `TC-CDS-013`
