...
sequenceDiagram
    ...
    TOOL->>PCDS: 0015 CommandRequest (K5) — Body 327B
    ...
    Note over TOOL,PCF: ★ 판정 — 알림 도착과 PDB 반영이 모두 맞아야 성공
        ...
        TOOL->>PDB: T_5G_SUBS_SERVICE 저장 확인 (MDN + R17 + N + K5 + TPID=0 + LIMIT=2 + LIMIT_VALID_TIME + CNUM)
        TOOL->>PDB: T_5G_RESERVED_JOB 저장 확인 (MDN + JOB_CODE=K7 + 핀)
    end
```
...
- `T_5G_SUBS_SERVICE` (MDN, `R17`, `SVC_TYPE=N`, `JOB_CODE=K5`, `TIME_PERIOD_ID=0`, `"LIMIT"=2`, `LIMIT_VALID_TIME`, `CNUM=핀`) = **1건 이상**
- `T_5G_RESERVED_JOB` (MDN, `JOB_CODE=K7`, 핀) = **1건 이상**
- **SBI Noti 도착** — `Verify SBI Noti Sent K5` (위 PDB 판정과 함께 본다)

알림은 `Verify SBI Noti Sent K5` 가 본다(도착 여부). 경로는 수신만으로 구분되지 않아 규칙으로 계산해 실패 메시지에 싣는다.

## 관련 문서

- [CDS 노드 스펙](../nodes/CDS.md) — 인코딩 표, 함정, LTE/SA 차이
- [1X 전문 End-to-End](CDS_X1.md) — PG 내부 프로세스 상세
- `tests/cds/cds_tests.robot` — `TC-CDS-009`
