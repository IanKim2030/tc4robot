...
| PG 측 프로세스 | **`G_LRS201`** — 이 슈트를 돌리기 전에 **기동 확인** (`/PG/CFG/ST.cfg`, `A_LRS201`/`G_LRS201`) |
...
> ★ **붙지 않으면 PG 쪽 `G_LRS201` 프로세스가 떠 있는지부터 본다.** 이 슈트의 두 채널
> (10204 / 8890)이 다 그 프로세스에 걸려 있어서, 안 떠 있으면 Setup 이 30초를 채우고
> 슈트가 서는 것 말고 다른 증상이 없다. 도구 쪽을 아무리 봐도 원인이 안 나온다.
...
- **PG 쪽 `G_LRS201` 이 떠 있어야 한다.** 안 떠 있으면 Suite Setup 이 8890 대기 30초를
  채우고 슈트가 선다 — 실패 메시지가 도구 쪽을 가리키므로 헤매기 쉽다.
