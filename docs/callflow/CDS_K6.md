...
sequenceDiagram
    ...
    TOOL->>PCDS: 0015 CommandRequest (K6) — Body 327B
...
- **SBI Noti 도착** — `Verify SBI Noti Sent K6` (위 PDB 판정과 함께 본다)

알림은 `Verify SBI Noti Sent K6` 가 본다(도착 여부). 경로는 수신만으로 구분되지 않아 규칙으로 계산해 실패 메시지에 싣는다.
