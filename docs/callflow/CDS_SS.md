...
| UPM 연동 | 없음 |
| 예약 큐 적재 | 없음 |
| Body 필드 수 | 4개 / 총 327B (고정) |

> 시간을 `LIMIT_VALID_TIME` 이 아니라 **`TIME_PERIOD_ID` 로 본다.** K1/K5/Y9 처럼 초 `00` 이 붙지 않는 12자리다 — 헷갈리는 자리다.
...
sequenceDiagram
    ...
    TOOL->>PCDS: 0015 CommandRequest (SS) — Body 327B
    ...
    Note over TOOL,PCF: ★ 판정 — 알림 도착과 PDB 반영이 모두 맞아야 성공
        ...
        TOOL->>PDB: T_5G_SUBS_SERVICE 저장 확인 (MDN + TIME_SVC_I + T + SS + TPID=SS_+START_TIME + LIMIT=0 + CNUM=0)
    end
```
...
- `T_5G_SUBS_SERVICE` (MDN, `TIME_SVC_I`, `SVC_TYPE=T`, `JOB_CODE=SS`, `TIME_PERIOD_ID='SS_' + START_TIME(12자리)`, `"LIMIT"=0`, `CNUM=0`) = **1건 이상**
- **SBI Noti 도착** — `Verify SBI Noti Sent SS` (위 PDB 판정과 함께 본다)

알림은 `Verify SBI Noti Sent SS` 가 본다(도착 여부). 경로는 수신만으로 구분되지 않아 규칙으로 계산해 실패 메시지에 싣는다.

## 관련 문서

- [CDS 노드 스펙](../nodes/CDS.md) — 인코딩 표, 함정, LTE/SA 차이
- [1X 전문 End-to-End](CDS_X1.md) — PG 내부 프로세스 상세
- `tests/cds/cds_tests.robot` — `TC-CDS-016`
