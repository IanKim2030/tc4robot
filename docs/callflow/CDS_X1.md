...
| PG | SNOTI | 가입자·세션 정보 조회 후 PCRF/PCF NOTI |
...
CDS로부터 1X 전문을 수신한 뒤, **PG.SDM/PG.SNOTI 계열(가입자 정보 처리)** 과 **PG.BSUBS 계열(HFC Cell 정보 처리)** 이 병행 동작하는 전체 흐름이다.
