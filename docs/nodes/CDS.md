...
| 코드 | PCF 로 알림 | 비고 |
|---|---|---|
| `A1` 신규가입 / `1Y` HFC해지 / `Z1` 가입해지 | **안 나간다** | 도구는 판정을 건너뛴다 |
| 그 외 전부 | 나간다 | 각 TC 가 `Verify SBI Noti Sent` 로 도착을 판정 |

`1X` 는 예외가 아니다 — `SDM → SNOTI` RBUS NOTI 가 안 나가 **SDM 발 가입자 통보**는
없지만, **SNOTI 를 BSUBS 가 깨운다**. `TC-CDS-003` 이 그 Cell List 를 판정한다.
